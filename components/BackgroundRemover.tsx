...
export default function BackgroundRemover({ lang }: { lang: string }) {
  ...
  // دالة مساعدة للنصوص الطويلة (تدعم العربية والإنجليزية كأساس، وتعود للإنجليزية لباقي اللغات)
  ...
  const faqs = [
    {
      ...
      answer: txt('نعم، الأداة مجانية 100% بدون أي رسوم مخفية أو قيود على عدد الصور.', 'Yes, the tool is 100% free with no hidden fees or limits.'),
    },
    {
      question: txt('هل صوري آمنة ومحمية؟', 'Are my images safe and secure?'),
      answer: txt('بالتأكيد! جميع المعالجة تتم مباشرة على متصفحك، ولا يتم رفع أي صورة إلى خوادمنا.', 'Absolutely! All processing happens directly in your browser, no images are uploaded.'),
    },
    {
      question: txt('ما هي الصيغة الأفضل للتحميل؟', 'What is the best format for download?'),
      answer: txt('لجودة عالية استخدم PNG. لسرعة تحميل المواقع استخدم WebP.', 'For high quality use PNG. For faster website loading use WebP.'),
    },
    {
      question: txt('هل يمكنني تغيير أبعاد الصورة؟', 'Can I resize the image?'),
      answer: txt('نعم! يمكنك تحديد العرض والارتفاع مع الحفاظ التلقائي على النسب.', 'Yes! You can specify width and height with automatic ratio preservation.'),
    },
    {
      question: txt('ما هي أنواع الصور المدعومة؟', 'What image types are supported?'),
      answer: txt('ندعم PNG، JPG، JPEG، WEBP بحد أقصى 10 ميجابايت.', 'We support PNG, JPG, JPEG, WEBP. Max size 10MB.'),
    },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex-1 text-center md:text-right">
            ...
            <p className="text-purple-100 text-sm md:text-base mb-3">
              {txt('استضافة سريعة وآمنة تبدأ من $2.99/شهر مع ضمان استعادة الأموال', 'Fast and secure hosting from $2.99/month with 30-day money-back guarantee')}
            </p>
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-xs">
              <span className="flex items-center gap-1"><Zap className="w-4 h-4" /> {txt('سرعة فائقة', 'Super Fast')}</span>
              <span className="flex items-center gap-1"><Shield className="w-4 h-4" /> {txt('SSL مجاني', 'Free SSL')}</span>
              <span className="flex items-center gap-1"><ImageIcon className="w-4 h-4" /> {txt('نطاق مجاني', 'Free Domain')}</span>
            </div>
          </div>
          <div className="flex-shrink-0">
            <div className="bg-gradient-to-r from-yellow-400 to-orange-500 text-gray-900 font-bold px-8 py-4 rounded-xl shadow-lg hover:scale-105 transition-transform text-center">
              ...
              <div className="text-sm opacity-90">{txt('سجل الآن ←', 'Sign Up Now →')}</div>
            </div>
          </div>
        </div>
      </a>
      ...
      <section className="bg-white rounded-2xl shadow-lg p-8 md:p-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { num: 2, title: txt('انتظر المعالجة', 'Wait'), desc: txt('ستتم إزالة الخلفية تلقائياً', 'Background removed automatically') },
            { num: 3, title: txt('عدّل الأبعاد', 'Resize'), desc: txt('غيّر العرض والارتفاع حسب احتياجك', 'Adjust width and height as needed') },
            { num: 4, title: txt('حمّل النتيجة', 'Download'), desc: txt('اختر PNG أو WebP وحمّل صورتك', 'Choose PNG or WebP and download') },
          ].map((item) => (
      ...
      {/* المميزات التفصيلية */}
      <section className="bg-white rounded-2xl shadow-lg p-8 md:p-12">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-8 text-center">⭐ {t('whyChoose')}</h2>
        <div className="grid md:grid-cols-2 gap-6">
          {[
            { icon: Zap, color: 'green', title: txt('معالجة فورية', 'Instant Processing'), desc: txt('إزالة الخلفية وتغيير الحجم في ثوانٍ', 'Remove background and resize in seconds') },
            { icon: Shield, color: 'blue', title: txt('خصوصية 100%', '100% Privacy'), desc: txt('لا يتم رفع صورك لأي خادم خارجي', 'Images are never uploaded to any server') },
            { icon: Crop, color: 'purple', title: txt('تغيير أبعاد مرن', 'Flexible Resizing'), desc: txt('تحكم كامل في الأبعاد مع الحفاظ على النسب', 'Full control over dimensions with ratio preservation') },
            { icon: ImageIcon, color: 'yellow', title: txt('صيغ متعددة', 'Multiple Formats'), desc: txt('حمّل بصيغة PNG أو WebP حسب احتياجك', 'Download in PNG or WebP format') },
            { icon: Star, color: 'red', title: txt('مجانية بدون حدود', 'Unlimited & Free'), desc: txt('استخدم الأداة مجاناً بدون أي قيود', 'Use the tool for free with no restrictions') },
            { icon: Check, color: 'indigo', title: txt('بدون تسجيل', 'No Signup'), desc: txt('ابدأ الاستخدام فوراً بدون حساب', 'Start using immediately without an account') },
          ].map((feature, idx) => (
      ...
      <section className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-8 md:p-12">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-8 text-center">📁 {t('pngVsWebp')}</h2>
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <h3 className="font-bold text-blue-900 mb-4 text-xl flex items-center gap-2"><ImageIcon className="w-6 h-6" /> PNG</h3>
            <ul className="space-y-3 text-sm text-gray-700">
              {[txt('جودة عالية بدون فقدان البيانات', 'High quality without data loss'), txt('مثالية للخلفيات الشفافة', 'Perfect for transparent backgrounds'), txt('مدعومة من جميع البرامج', 'Supported by all software')].map((item, i) => (
                <li key={i} className="flex items-start gap-2"><Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" /><span>{item}</span></li>
              ))}
            </ul>
          </div>
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <h3 className="font-bold text-green-900 mb-4 text-xl flex items-center gap-2"><Zap className="w-6 h-6" /> WebP</h3>
            <ul className="space-y-3 text-sm text-gray-700">
              {[txt('حجم أصغر بنسبة 25-35% من PNG', '25-35% smaller than PNG'), txt('مثالية لمواقع الويب', 'Perfect for websites'), txt('موصى بها من Google', 'Recommended by Google')].map((item, i) => (
                <li key={i} className="flex items-start gap-2"><Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" /><span>{item}</span></li>
              ))}
            </ul>
          </div>
      ...
      {/* الاستخدامات */}
      <section className="bg-white rounded-2xl shadow-lg p-8 md:p-12">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-8 text-center">🎯 {t('uses')}</h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { icon: '📸', title: txt('التصوير الفوتوغرافي', 'Photography'), desc: txt('صور شخصية وعائلية احترافية', 'Professional personal photos') },
            ...
            { icon: '🎨', title: txt('التصميم الجرافيكي', 'Graphic Design'), desc: txt('عزل العناصر للتصاميم', 'Isolate elements for designs') },
            { icon: '📱', title: txt('السوشيال ميديا', 'Social Media'), desc: txt('منشورات جذابة لإنستغرام وتيك توك', 'Attractive posts for Instagram & TikTok') },
            { icon: '🎓', title: txt('السيرة الذاتية', 'Resume/CV'), desc: txt('صور احترافية للسيرة الذاتية', 'Professional photos for your CV') },
        ...
        <div className="space-y-3 max-w-4xl mx-auto">
          {faqs.map((faq, idx) => (
            <div key={idx} className="border border-gray-200 rounded-lg overflow-hidden">
              <button onClick={() => setOpenFaq(openFaq === idx ? null : idx)} className="w-full flex items-center justify-between p-5 text-right hover:bg-gray-50 transition-colors">
      ...
      <section className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 md:p-12 text-white text-center">
        ...
        <p className="text-lg mb-6 opacity-90">{txt('ابدأ الآن مجاناً - بدون تسجيل، بدون حدود', 'Start now for free - no signup, no limits')}</p>
        <button onClick={() => { reset(); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="bg-white text-blue-600 font-bold px-8 py-4 rounded-xl hover:scale-105 transition-transform shadow-lg">
          🚀 {t('startNow')}
        </button>
      </section>
      ...
      <footer className="bg-gray-100 rounded-2xl p-8 text-center text-gray-600">
        <p className="mb-2 font-semibold">{txt('أداة إزالة خلفية الصور - مجانية 100% وآمنة', 'Background Remover Tool - 100% Free & Secure')}</p>
        <p className="text-sm mb-4">{txt('جميع المعالجة تتم على متصفحك مباشرة.', 'All processing happens directly in your browser.')}</p>
        <p className="text-xs">© {new Date().getFullYear()} Free Background Remover - All rights reserved</p>
      </footer>
    </div>
  );
}
